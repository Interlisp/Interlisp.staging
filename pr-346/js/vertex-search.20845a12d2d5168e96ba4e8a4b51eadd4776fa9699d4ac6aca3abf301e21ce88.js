...
`;
  ...
  (function() {
    ...
    function setMode(mode, options2) {
      const updateUrl = !options2 || options2.updateUrl !== false;
    ...
    function renderSummaryMarkdown(summaryText, citationCount) {
      ...
      const html3 = marked.parse(linkedMarkdown);
    ...
    function renderResultSnippet(snippetText, result) {
      ...
      const html3 = marked.parse(normalized);
      return purify.sanitize(html3, {
    ...
    function decodeHtmlEntities(text2) {
      ...
      textarea.innerHTML = String(text2 || "");
      return textarea.value;
    }
    function normalizeSnippetText(text2, result) {
      let output = decodeHtmlEntities(text2);
      output = output.replace(
        /(^|[\s(\[{])b([^\n]{1,120}?)\/b(?=[\s)\]}.,;:!?]|$)/gi,
        (match, prefix, value) => `${prefix}<strong>${value.trim()}</strong>`
      );
      output = output.replace(/https?:\/\/b/gi, "https://").replace(/\bb([a-z0-9.-]+)\/b/gi, "$1").replace(/\/(b)([a-z0-9._-]+)/gi, "/$2").replace(/([a-z0-9._-]+)\/b(?=\/|\b)/gi, "$1").replace(/\s*\.\.\.\s*/g, " ");
      output = normalizeGithubIssueMentions(output, result);
      output = output.replace(/https?:\/\/\S+$/i, "");
      output = output.replace(/\s+/g, " ").trim();
      return output;
    }
    function normalizeGithubIssueMentions(text2, result) {
      const repo = result?.repo;
      if (!repo) return text2;
      let output = text2.replace(
        /\[(Issue\s+#?\d+)\]\((https?:\/\/[^)]+)\)/gi,
        (match, label, rawUrl) => {
          const issueNumberMatch = label.match(/(\d+)/);
          if (!issueNumberMatch) return match;
          const issueNumber = issueNumberMatch[1];
          const cleanUrl2 = `https://github.com/Interlisp/${repo}/issues/${issueNumber}`;
          return `[${label}](${cleanUrl2})`;
        }
      );
    ...
    async function doSearch(q) {
      ...
      statusEl.style.display = "";
      statusEl.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Searching for <strong>' + escapeHtml(q) + "</strong>\u2026";
      try {
        ...
        if (data.summary?.summaryText) {
          ...
          if (validRefs.length > 0) {
            const citationHtml = validRefs.map((ref2, i) => `
            <div id="source-row-${i + 1}" class="search-citation">
              <span class="citation-number">[${i + 1}]</span>
              ${ref2.uri ? `<a href="${escapeHtml(ref2.uri)}" target="_blank" rel="noopener">${escapeHtml(ref2.title || ref2.uri)}</a>` : `<span>${escapeHtml(ref2.title || "Unknown source")}</span>`}
            </div>
          `).join("");
        ...
        hitsEl.innerHTML = `<p class="text-muted mb-3">${data.results.length} results for <strong>${escapeHtml(q)}</strong></p>` + data.results.map((r) => `
        <div class="td-search-hit mb-4">
          ...
          ${r.type || r.repo || r.state ? `<p class="td-search-hit__meta mb-0">
